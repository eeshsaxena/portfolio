...
Standard knowledge distillation transfers soft label distributions. **Spectral distillation** additionally aligns the frequency-domain representations of intermediate feature maps, which is crucial for preserving temporal cardiac signal structure that would otherwise be lost during aggressive compression.
...
|--------|---------|---------|-------|
| Parameters | 4.2M | 30K | 140x |
| Model size | 16.8 MB | 120 KB | 140x |
| Inference time | 45ms (GPU) | 38ms (Arduino) | n/a |
| F1 Score | 99.1% | 98.7% | -0.4% |

**140x compression with only 0.4% F1 loss**, making clinical-grade arrhythmia detection feasible on a $30 device.
...
This project shows that **edge AI for healthcare** is achievable without expensive hardware. The same approach extends to seizure detection, sleep apnea monitoring, and other continuous physiological monitoring.
